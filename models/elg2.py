...
class ELG(BaseModel):
    ...
    def build_model(self, data_sources: Dict[str, BaseDataSource], mode: str):
        ...
        #transform_stage
        te = h2
        with tf.variable_scope('transform_stage'):
            te = tf.transpose(te, (0, 2, 3, 1))
            te = tf.image.resize_bilinear(te, [9,15], align_corners=True)
            te = tf.image.resize_bilinear(te, [18,30], align_corners=True)
            te = tf.image.resize_bilinear(te, [36,60], align_corners=True)
            te = tf.transpose(te, (0, 3, 1, 2))
        ...
        with tf.variable_scope('hourglass'):
            ...
            for i in range(self._hg_num_modules):
                with tf.variable_scope('hg_%d' % (i + 1)):
                    ...
                    #print(h)
                    '''
                    with tf.variable_scope('x_transform'):
                        x = self._apply_conv(x, num_features=self._hg_num_feature_maps, kernel_size=3, stride=1)
                    with tf.variable_scope('h_transform'):
                        h = self._apply_conv(h, num_features=self._hg_num_landmarks, kernel_size=3, stride=2)'''
                    ...
                    result_mse = 0.9*h+0.1*te

                    if y1 is not None:
                        metrics['heatmap%d_mse' % (i + 1)] = _tf_mse(result_mse, y1)
